  ...
  template: `
    <div *ngIf="isAdmin"
         class="fixed bottom-5 left-1/2 -translate-x-1/2 z-50
                ...
                text-xs font-semibold select-none animate-slide-up transition-all duration-300"
         [class]="editMode
      ...
      <ng-container *ngIf="!editMode">
        <button (click)="startEdit()" class="flex items-center gap-1.5 hover:text-[#7dd87a] transition-colors cursor-pointer">
      ...
      <ng-container *ngIf="editMode">
        <button (click)="emitSave()" class="flex items-center gap-1.5 font-bold hover:text-white/80 transition-colors cursor-pointer">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span class="w-px h-4 bg-white/30"></span>
        <button (click)="emitCancel()" class="flex items-center gap-1.5 hover:text-white/70 transition-colors cursor-pointer">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
